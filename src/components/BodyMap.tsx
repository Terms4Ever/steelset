...
export function BodyMap({ volumes }: { volumes: Record<string, number> }) {
  const max = Math.max(1, ...Object.values(volumes));
  const v = (m: MuscleGroup) => colorFor((volumes[m] ?? 0) / max);
    ...
    <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'flex-end' }}>
      <Figure label="Zepředu">
        <Svg width={120} height={260} viewBox="0 0 120 260">
          ...
          {/* quads */}
          <Ellipse cx={49} cy={176} rx={14} ry={34} fill={v('Nohy')} />
          <Ellipse cx={71} cy={176} rx={14} ry={34} fill={v('Nohy')} />
          {/* shins */}
      ...
      <Figure label="Zezadu">
        <Svg width={120} height={260} viewBox="0 0 120 260">
          ...
          {/* hamstrings (Nohy) */}
          <Ellipse cx={49} cy={176} rx={14} ry={32} fill={v('Nohy')} />
          <Ellipse cx={71} cy={176} rx={14} ry={32} fill={v('Nohy')} />
          {/* calves */}
